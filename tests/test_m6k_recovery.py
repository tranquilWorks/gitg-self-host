...
def test_recovered_corpus_and_protected_runtime_are_exact():
...
def test_initial_ledger_cannot_be_reclassified_as_acceptance(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(recovery, "invariant_snapshot", lambda root: baseline)
...
def test_protected_rule_change_cannot_rebaseline_itself(monkeypatch):
    ...
    monkeypatch.setattr(recovery, "invariant_snapshot", lambda root: changed)
